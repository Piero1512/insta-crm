...
export default function LeadsPage() {
  ...
  const handleQuickInteraction = async (leadId: string, type: 'call' | 'whatsapp') => {
    // Si aún no tienes auth configurado, usamos un UUID placeholder o null para coordinator_id
    // Para la prueba insertamos directo en lead_interactions o actualizamos contadores
    const { error } = await supabase.rpc('log_lead_contact', {
      p_lead_id: leadId,
      p_type: type,
    });

    // En caso de que el RPC no esté definido, hacemos update directo como fallback:
    if (error) {
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden border border-slate-200">
            ...
            <form onSubmit={handleCreateLead} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ej. 123 NW 36th St, Miami, FL"
                  value={address}
